<template>
  <workspace-page-shell :title="title" :description="description" @refresh="refreshContent">
    <section class="forum-helper-panel">
      <div>
        <h3>开始一条新的讨论</h3>
        <p>{{ helperText }}</p>
      </div>
      <div class="forum-helper-actions">
        <el-button plain @click="goToForumManage">{{ manageText }}</el-button>
        <el-button @click="goToForum">前往论坛</el-button>
      </div>
    </section>
...
const manageText = computed(() => (canModerateForumPosts.value ? '管理论坛帖子' : '管理我的帖子'))

const goToForum = () => router.push('/forum')
const goToForumManage = () => router.push(`${routeBase.value}/forum-manage`)
