<template>
  <workspace-page-shell title="账号设置" :loading="loading" @refresh="loadProfile">
    <overview-section-card
      title="当前资料"
      description="这些信息来自当前登录账号，可帮助确认资料同步是否正确。"
    >
    ...
    <div class="settings-grid">
      <overview-section-card title="头像设置">
        <div class="avatar-section">
          <el-avatar :size="88" :src="avatarForm.avatar || currentProfile.avatar">{{
            avatarInitial
          }}</el-avatar>
          ...
          <div class="avatar-form">
            ...
            <div class="avatar-actions">
              ...
              <el-button type="primary" :loading="avatarSaving" @click="submitAvatar"
                >保存头像</el-button
              >
            </div>
          </div>
      ...
      <overview-section-card
        title="密码设置"
        description="首次使用 GitHub 登录的账号，可以在这里补充站内密码。"
      >
...
const avatarInitial = computed(
  () => currentProfile.value.nickname?.slice(0, 1)?.toUpperCase() || 'D',
)
...
const handleAvatarUpload = async (options) => {
  ...
  try {
    ...
    avatarForm.avatar = relativePath.startsWith('http')
      ? relativePath
      : `${uploadBaseUrl}${relativePath}`
